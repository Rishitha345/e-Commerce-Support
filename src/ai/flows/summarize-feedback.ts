...
'use server';
/**
 * @fileOverview A flow that summarizes customer feedback from chat transcripts.
 *
...
const SummarizeFeedbackInputSchema = z.object({
  chatTranscript: z
    .string()
    .describe('The chat transcript to summarize.'),
});
...
const SummarizeFeedbackOutputSchema = z.object({
  summary: z.string().describe('The summary of the chat transcript.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant helping to summarize customer feedback from chat transcripts.

  Please provide a concise summary of the following chat transcript, highlighting the main issues and areas for improvement that the customer mentioned:
